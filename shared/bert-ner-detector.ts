...
function resolveScriptPath(): string {
  if (process.env.PII_BERT_SCRIPT) return process.env.PII_BERT_SCRIPT;
  return path.join(app.getAppPath(), 'scripts', 'bert_ner.py');
}

export class BertNerDetector implements PIIDetector {
  async collectMatches(text: string): Promise<RawMatch[]> {
    if (!text.trim()) return [];
    if (process.env.PII_BERT_DISABLE === '1') return [];
    ...
    const pythonBin = process.env.PII_BERT_PY ?? 'python3';
    ...
    return new Promise((resolve, reject) => {
      ...
      proc.on('close', (code, signal) => {
        ...
        if (code !== 0) {
          ...
          reject(new Error(stderr || `BERT NER process exited with code ${code}`));
          return;
        }

        try {
          const entities = JSON.parse(stdout) as BertEntity[];
          ...
          const nonOverlapping = selectNonOverlapping(matches);
          log('done', { entities: entities.length, matches: nonOverlapping.length });
          resolve(nonOverlapping);
        } catch (err) {
