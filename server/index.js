...
import { applyAllowlist, collectManualMatches, maskText, reconstructMatches } from '../shared/scanner';
...
const layerState = Object.fromEntries(piiDetectors.map((detector) => [detector.getName(), true]));
...
async function handleScan(payload) {
  ...
  for (const m of manualMatches) allDetections.push({ value: m.value, source: m.source, type: m.type, score: m.score });
  ...
  for (const m of regexMatches) allDetections.push({ value: m.value, source: m.source, type: m.type, score: m.score });
  ...
  const nerResultArrays = nerServerReady
    ? await Promise.all(
        activeNer.map(async (d) => {
          ...
          return applyAllowlist(stage1Text, raw, allowlist);
        })
      )
    : [];

  const allNerMatches = nerResultArrays.flat();
  for (const m of allNerMatches) allDetections.push({ value: m.value, source: m.source, type: m.type, score: m.score });
  ...
  if (activeLlm) {
    ...
    for (const m of llmMatches) allDetections.push({ value: m.value, source: m.source, type: m.type, score: m.score });
  }
  ...
  const llmTokens =
    activeLlm && 'getLastEvalCount' in activeLlm ? activeLlm.getLastEvalCount?.() : undefined;
  const llmElapsedMs =
    activeLlm && 'getLastElapsedMs' in activeLlm ? activeLlm.getLastElapsedMs?.() : undefined;
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (u.pathname === '/api/layers/set' && req.method === 'POST') {
      ...
      if (name in layerState) layerState[name] = enabled;
      sendJson(res, 200, { ...layerState });
      return;
    }
    ...
    if (fs.existsSync(distDir)) {
      ...
      if (filePath.startsWith(distDir) && fs.existsSync(filePath) && fs.statSync(filePath).isFile()) {
        ...
        res.writeHead(200, { 'Content-Type': guessContentType(filePath), 'Content-Length': buf.length });
        res.end(buf);
        return;
      }
